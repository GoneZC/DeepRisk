...
class BaseDeepAD(metaclass=ABCMeta):
    ...
    def fit(self, X, y=None, train_loader=None, net=None, criterion=None):
        """
        ...
        for _ in range(self.n_ensemble):
            if self.model_name ==  'MultiSAD':
                self.train_loader = train_loader
                self.net = net
                self.criterion = criterion 
            elif self.model_name == 'DeepSAD':
                self.train_loader, self.net, self.criterion, _ = self.training_prepare(X=self.train_data,
                                                                                       y=self.train_label)
            else:
                self.train_loader, self.net, self.criterion = self.training_prepare(X=self.train_data,
                                                                                       y=self.train_label)
            self._training()
